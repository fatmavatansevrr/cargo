...
/**
 * Analytics Controller - Requirements FR-AN-001 to FR-AN-005
 * Kargo takip sistemi için analitik verilerin yönetimi
 */
@RestController
...
public class AnalyticsController {
    ...
    public ResponseEntity<ShipmentAnalyticsDTO> saveAnalytics(
        ...
        try {
            System.out.println("Analytics data save request: shipmentId=" + analyticsDTO.getShipmentId());
            return ResponseEntity.ok(analyticsMapper.toDTO(
                analyticsService.saveAnalytics(analyticsMapper.toEntity(analyticsDTO))
            ));
        } catch (Exception e) {
            System.err.println("Analytics data save error: " + e.getMessage());
            e.printStackTrace();
    ...
    public ResponseEntity<List<ShipmentAnalyticsDTO>> getAnalyticsByCarrier(
        ...
        try {
            System.out.println("Analytics by carrier request: carrierId=" + carrierId);
            return ResponseEntity.ok(analyticsMapper.toDTOList(
                analyticsService.getAnalyticsByCarrier(carrierId)
            ));
        } catch (Exception e) {
            System.err.println("Analytics by carrier error: " + e.getMessage());
            e.printStackTrace();
            return ResponseEntity.internalServerError().build();
        }
    }
    ...
    public ResponseEntity<List<ShipmentAnalyticsDTO>> getAnalyticsByShipper(
        ...
        try {
            System.out.println("Analytics by shipper request: shipperId=" + shipperId);
            return ResponseEntity.ok(analyticsMapper.toDTOList(
                analyticsService.getAnalyticsByShipper(shipperId)
            ));
        } catch (Exception e) {
            System.err.println("Analytics by shipper error: " + e.getMessage());
            e.printStackTrace();
            return ResponseEntity.internalServerError().build();
        }
    }
    ...
    public ResponseEntity<List<ShipmentAnalyticsDTO>> getAnalyticsByCustomer(
        ...
        try {
            System.out.println("Analytics by customer request: customerId=" + customerId);
            return ResponseEntity.ok(analyticsMapper.toDTOList(
                analyticsService.getAnalyticsByCustomer(customerId)
            ));
        } catch (Exception e) {
            System.err.println("Analytics by customer error: " + e.getMessage());
            e.printStackTrace();
            return ResponseEntity.internalServerError().build();
        }
    }
    ...
    public ResponseEntity<List<ShipmentAnalyticsDTO>> getAnalyticsByDateRange(
        ...
        try {
            System.out.println("Analytics by date range request: startDate=" + startDate + ", endDate=" + endDate);
            return ResponseEntity.ok(analyticsMapper.toDTOList(
                analyticsService.getAnalyticsByDateRange(startDate, endDate)
            ));
        } catch (Exception e) {
            System.err.println("Analytics by date range error: " + e.getMessage());
            e.printStackTrace();
            return ResponseEntity.internalServerError().build();
        }
    }
    ...
    public ResponseEntity<CarrierPerformanceDTO> getCarrierPerformance(
            ...
            @PathVariable String carrierId) {
        try {
            System.out.println("Carrier performance request: carrierId=" + carrierId);
            return ResponseEntity.ok(analyticsMapper.toCarrierPerformanceDTO(
                carrierId,
                analyticsService.getAnalyticsByCarrier(carrierId)
            ));
        } catch (Exception e) {
            System.err.println("Carrier performance error: " + e.getMessage());
            e.printStackTrace();
            return ResponseEntity.internalServerError().build();
        }
    }
    ...
    public ResponseEntity<StatusDistributionDTO> getStatusDistribution() {
        try {
            System.out.println("Status distribution request");
            return ResponseEntity.ok(analyticsMapper.toStatusDistributionDTO(
                analyticsService.getAllAnalytics()
            ));
        } catch (Exception e) {
            System.err.println("Status distribution error: " + e.getMessage());
            e.printStackTrace();
            return ResponseEntity.internalServerError().build();
        }
    }
